// Learn more about F# at http://docs.microsoft.com/dotnet/fsharp

open Year2020Day1
open Year2020Day2
open Year2020Day3
open Year2020Day4
open Year2020Day5
open Year2020Day6
open Year2020Day7
open Year2020Day8
open Year2020Day9
open Year2020Day10
open Year2020Day11
open Year2020Day12
    
[<EntryPoint>]
let main argv =
    printfn "Day 1 part 1 solution is %i" SolveDay1Part1
    printfn "Day 1 part 2 solution is %i" SolveDay1Part2
    printfn "---"
    printfn "Day 2 part 1 solution is %i" SolveDay2Part1
    printfn "Day 2 part 2 solution is %i" SolveDay2Part2
    printfn "---"
    printfn "Day 3 part 1 solution is %i" SolveDay3Part1
    printfn "Day 3 part 2 solution is %i" SolveDay3Part2
    printfn "---"
    printfn "Day 4 part 1 solution is %i" SolveDay4Part1
    printfn "Day 4 part 2 solution is %i" SolveDay4Part2
    printfn "---"
    printfn "Day 5 part 1 solution is %i" SolveDay5Part1
    printfn "Day 5 part 2 solution is %A" SolveDay5Part2
    printfn "---"
    printfn "Day 6 part 1 solution is %i" SolveDay6Part1
    printfn "Day 6 part 2 solution is %i" SolveDay6Part2
    printfn "---"
    printfn "Day 7 part 1 solution is %A" SolveDay7Part1 //Probably needs performance improvements
    printfn "Day 7 part 1 solution is %A" SolveDay7Part2 //Incomplete
    printfn "---"
    printfn "Day 8 part 1 solution is %A" SolveDay8Part1
    printfn "Day 8 part 2 solution is %A" SolveDay8Part2
    printfn "---"
    printfn "Day 9 part 1 solution is %A" SolveDay9Part1
    printfn "Day 9 part 2 solution is %A" SolveDay9Part2
    printfn "---"
    printfn "Day 10 part 1 solution is %A" SolveDay10Part1
    printfn "Day 10 part 2 solution is %A" SolveDay10Part2 //Incomplete
    printfn "---"
    printfn "Day 11 part 1 solution is %A" SolveDay11Part1
    printfn "Day 11 part 2 solution is %A" SolveDay11Part2 
    printfn "---"
    printfn "Day 12 part 1 solution is %A" SolveDay12Part1
    printfn "Day 12 part 2 solution is %A" SolveDay12Part2 //Needs refactoring, lots of duplicated code... kind of ugly
    0
